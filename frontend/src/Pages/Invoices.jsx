...
const EmptyState = ({ search }) => (
  <div className="flex flex-col items-center justify-center py-20 gap-3 text-neutral-400 dark:text-neutral-500">
    ...
    <p className="text-sm">
      {!search &&
        "Invoices are created automatically when you create a sales order"}
    </p>
  </div>
);
...
const Invoices = () => {
  ...
  return (
    <>
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-neutral-500 dark:text-neutral-400 text-sm mt-1">
                Auto-generated from sales orders — update payment status here
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            {[
              ...
              >
                ...
                <p
                  className={`text-xl font-bold bg-gradient-to-r ${s.color} bg-clip-text text-transparent mt-1`}
                >
                  {s.value}
                </p>
              </div>
                  ...
                  <tbody>
                    {filtered.map((inv) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 text-green-600 dark:text-green-400">
                          ৳ {inv.advanceAmount?.toLocaleString()}
                        </td>
                        <td className="px-5 py-3.5 text-red-500 dark:text-red-400">
                          ৳ {inv.dueAmount?.toLocaleString()}
                        </td>
                        <td className="px-5 py-3.5">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-semibold ${statusStyle(inv.status)}`}
                          >
                            {inv.status}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex justify-center gap-2">
                            <button
                              ...
                              className="p-1.5 rounded-lg hover:bg-amber-50 dark:hover:bg-amber-900/20 text-amber-500 cursor-pointer transition"
                            >
                              <Eye size={15} />
                            </button>
                            <button
      ...
      {pdfInvoice && <InvoicePDF inv={pdfInvoice} handleClose={handleClose} />}
    </>
  );
};
