...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center py-20 gap-3 text-neutral-400 dark:text-neutral-500">
    ...
    <p className="text-sm">
      Inventory updates automatically when purchase orders are marked as
      Received
    </p>
  </div>
);
...
const AdjustModal = ({ item, handleClose, setInventory }) => {
  ...
  const inp =
    "w-full rounded-lg border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-700 text-neutral-800 dark:text-neutral-100 py-2 px-3 text-sm outline-none focus:ring-2 focus:ring-amber-400 transition";
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(
        `Stock ${type === "add" ? "added" : "reduced"} successfully`,
      );
      handleClose();
    } catch (err) {
        ...
        <div className="flex justify-between items-center border-b border-neutral-200 dark:border-neutral-700 p-5">
          <div>
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
              {item.product?.name} · Current: {item.currentStock} {item.unit}
            </p>
          </div>
          <button
            onClick={handleClose}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() => setType("add")}
                  className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition cursor-pointer ${type === "add" ? "bg-green-500 text-white" : "border border-neutral-300 dark:border-neutral-600 text-neutral-600 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-700"}`}
                >
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition cursor-pointer ${type === "subtract" ? "bg-red-500 text-white" : "border border-neutral-300 dark:border-neutral-600 text-neutral-600 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-700"}`}
                >
  ...
  return (
    <>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-neutral-500 dark:text-neutral-400 text-sm mt-1">
                Track finished product stock levels
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            {[
              ...
              {
                label: "In Stock",
                value: inventory.filter((i) => i.currentStock > 0).length,
              ...
              <div
                key={s.label}
                className="bg-white dark:bg-neutral-800/70 rounded-2xl border border-neutral-200 dark:border-neutral-700/50 p-4 shadow-sm"
              >
                <p className="text-xs text-neutral-500 dark:text-neutral-400">
                  {s.label}
                </p>
                <p
                  className={`text-2xl font-bold bg-gradient-to-r ${s.color} bg-clip-text text-transparent mt-1`}
                >
                  {s.value}
                </p>
              </div>
                  ...
                  <thead>
                    <tr className="bg-neutral-50 dark:bg-neutral-700/40 border-b border-neutral-200 dark:border-neutral-700 text-left">
                      {[
                        ...
                        <th
                          key={h}
                          className={`px-5 py-3 font-medium text-neutral-500 dark:text-neutral-400 ${h === "Adjust" ? "text-center" : ""}`}
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {filtered.map((item, i) => (
                      ...
                      >
                        ...
                        <td
                          className={`px-5 py-3.5 ${stockStyle(item.currentStock)}`}
                        >
                        ...
                        <td className="px-5 py-3.5 text-center">
                          <button
                            ...
                            className="px-3 py-1.5 text-xs font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-lg hover:bg-amber-200 dark:hover:bg-amber-900/50 cursor-pointer transition"
                          >
                            Adjust
                          </button>
                        </td>
      ...
      {adjustItem && (
        <AdjustModal
          item={adjustItem}
          handleClose={() => setAdjustItem(null)}
