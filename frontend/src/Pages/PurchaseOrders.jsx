...
const EmptyState = ({ search }) => (
  <div className="flex flex-col items-center justify-center py-20 gap-3 text-neutral-400 dark:text-neutral-500">
    ...
    <p className="text-base font-semibold text-neutral-600 dark:text-neutral-300">{search ? "No orders match your search" : "No purchase orders yet"}</p>
...
const PurchaseOrders = () => {
  ...
  useEffect(() => { getPurchaseOrders().then(setOrders).catch((e) => toast.error(e.message)).finally(() => setLoading(false)); }, []);
  useEffect(() => {
  ...
  const handleDelete = (id) => {
    Swal.fire({ title: "Are you sure?", icon: "warning", showCancelButton: true, confirmButtonColor: "#f59e0b", cancelButtonColor: "#d33", confirmButtonText: "Yes, delete!" })
      .then(async (r) => { if (r.isConfirmed) { await deletePurchaseOrder(id); setOrders((p) => p.filter((o) => o._id !== id)); Swal.fire("Deleted!", "", "success"); } });
  };
  ...
  return (
    <>
      <section className="p-5 md:p-8 min-h-screen bg-neutral-50 dark:bg-neutral-950 transition-colors duration-300">
        <div className="max-w-6xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-neutral-800 dark:text-white">Purchase Orders</h1>
              <p className="text-neutral-500 dark:text-neutral-400 text-sm mt-1">Manage all supplier purchase orders</p>
            </div>
          ...
          <div className="mb-4">
            <input type="text" placeholder="Search by order no or supplier..." value={search} onChange={(e) => setSearch(e.target.value)}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-neutral-50 dark:bg-neutral-700/40 border-b border-neutral-200 dark:border-neutral-700 text-left">
                        {["Order No", "Supplier", "Items", "Total (৳)", "Expected Date", "Status", "Date", "Actions"].map((h) => (
                    ...
                    <tbody>
                      {filtered.map((o) => (
                        <tr key={o._id} className="border-t border-neutral-100 dark:border-neutral-700/40 hover:bg-neutral-50 dark:hover:bg-neutral-700/20 transition">
                          ...
                          <td className="px-5 py-3.5 font-medium text-neutral-800 dark:text-neutral-100">{o.supplier?.name || "—"}</td>
                          ...
                          <td className="px-5 py-3.5 text-neutral-500 dark:text-neutral-400">{o.expectedDate ? new Date(o.expectedDate).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "—"}</td>
      ...
      {isCreate && <PurchaseOrderModal handleClose={handleClose} setOrders={setOrders} mode="create" />}
      {isEdit && selected && <PurchaseOrderModal handleClose={handleClose} setOrders={setOrders} mode="edit" selected={selected} />}
    </>
  );
};
