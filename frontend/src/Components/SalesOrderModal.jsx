...
const SalesOrderModal = ({ handleClose, setOrders, mode, selected }) => {
  ...
  const calcTotal = (items) => items.reduce((acc, i) => acc + (Number(i.totalPrice) || 0), 0);
    ...
    <section className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50">
      <div className="bg-white dark:bg-neutral-800 w-full max-w-2xl max-h-[88vh] overflow-y-auto rounded-2xl mx-4 shadow-2xl">
        <div className="sticky top-0 flex justify-between items-start border-b border-neutral-200 dark:border-neutral-700 p-5 bg-white dark:bg-neutral-800 z-10">
          <div>
            <h1 className="text-lg font-bold text-neutral-800 dark:text-white">{mode === "edit" ? "Update Sales Order" : "Create Sales Order"}</h1>
            <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">Fill in the order details — an invoice will be created automatically</p>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            <div className="bg-neutral-50 dark:bg-neutral-700/30 rounded-xl p-4">
              ...
              <div><Label>Customer *</Label>
                <select required value={form.customer} onChange={(e) => setForm((p) => ({ ...p, customer: e.target.value }))} className={inp}>
                  <option value="">— Select customer —</option>
                  {customers.map((c) => <option key={c._id} value={c._id}>{c.name}{c.businessName ? ` · ${c.businessName}` : ""}</option>)}
                </select>
                {customers.length === 0 && <p className="text-xs text-amber-500 mt-1">No active customers. Add a customer first.</p>}
              </div>
            </div>
            ...
            <div className="bg-neutral-50 dark:bg-neutral-700/30 rounded-xl p-4">
              ...
              <div className="space-y-3">
                {form.items.map((item, idx) => (
                  <div key={idx} className="bg-white dark:bg-neutral-700/50 border border-neutral-200 dark:border-neutral-600 rounded-xl p-4">
                    <div className="flex justify-between items-center mb-3">
                    ...
                    {products.length > 0 && (
                      <div className="mb-3"><Label>Select Product (auto-fills below)</Label>
            ...
            <div className="bg-neutral-50 dark:bg-neutral-700/30 rounded-xl p-4">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div><Label>Advance Amount (৳)</Label><input type="number" min="0" value={form.advanceAmount} onChange={(e) => setForm((p) => ({ ...p, advanceAmount: e.target.value }))} className={inp} placeholder="0" /></div>
              </div>
              <div className="mb-3"><Label>Due Amount (৳)</Label><input readOnly value={Math.max(0, (Number(form.totalAmount) || 0) - (Number(form.advanceAmount) || 0))} className={`${inp} bg-neutral-100 dark:bg-neutral-600 cursor-not-allowed`} /></div>
